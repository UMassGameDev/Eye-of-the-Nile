...
{
    public SpriteRenderer sRenderer;  // Reference to the object’s sprite renderer.
    public Animator animator;  // Reference to the object’s animator.
    public Transform hurtEffect;  // Reference to the particle effects the object should spawn when taking damage.
    public SpriteRenderer onFireSprite;  // Reference to the sprite that should be overlaid the object’s sprite when the object is on fire.
    public string deathSfxName;  // Reference to the sound effect the object should play when it dies.

    public bool IsDead { get { return currentHealth <= 0; } }  // The object is dead if the currentHealth falls to 0.
    public bool enableDamageParticles = true;  // If enabled, the object will spawn the given hurtEffect particles when taking damage.

    public int maxHealth = 100;  // The maximum health the object can have.
    public int currentHealth { get; protected set; }  // The maximum health the object can have.

    public int soulsDroppedOnDeath = 0;  // The amount of souls the object should grant the player upon death. To disable, set to <= 0.
    public int godSoulsDroppedOnDeath = 0;  // The amount of god souls the object should grant the player upon death. To disable, set to <= 0.
    ...
    public bool canBeInvincible = false;  // If enabled, the object will be invincible for a short duration after taking damage (invincibility frames)
    public float invincibleDuration = 3f;  // In seconds, how long invincibility lasts for after taking damage.
    protected float flashDuration = 0.25f;  // In seconds, how often the sprite should swap between being opaque and transparent (creating a flashing effect).
    protected WaitForSeconds invincibleFlash;  // The WaitForSeconds variable that is used to implement flashDuration.
    protected bool isInvincible = false;  // If the object is currently invincible.

    public bool canBeOnFire = true;  // Whether or not the object can be set on fire.
    protected bool onFire = false;  // Whether or not the object is currently on fire.
    protected bool fireImmune = false;  // Whether or not the object currently has immunity to fire (usually from an ability).
    ...
    // Triggered when the object runs out of health.
    // Plays a death sound and animation, drops souls/god souls, and disables the object.
